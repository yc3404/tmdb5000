...
credits_cast <- credits %>% select(-crew) %>% filter(nchar(cast) >2) %>%  mutate(cast=lapply(cast,fromJSON)) %>% unnest(cast) %>% select(movie_id,title,cast=name) %>% mutate(id=movie_id)%>%mutate(movie_id=NULL) %>% group_by(id, title) %>% mutate(cast = paste0(cast, collapse = " ")) %>% distinct()
